#ifndef _mj_arraylist_h_
#define _mj_arraylist_h_
...
#define ARRAY_INIT(type, arr, size)                                                      \
  (arr)->capacity = size;                                                                \
  (arr)->length = 0;                                                                     \
  (arr)->array = (type *)my_malloc((size) * sizeof( type ));                             \
  memset((arr)->array,0,(size) * sizeof( type ))

#define ARRAY_DESTROY(type, arr)     my_free((arr)->array)
